...
static inline float mix(float a, float b, float p) { return a + (b-a)*p; }
...
float DelunayTriangle::getMinX() const { return min(min(a->rx, b->rx), c->rx); }
float DelunayTriangle::getMaxX() const { return max(max(a->rx, b->rx), c->rx); }
float DelunayTriangle::getMinY() const { return min(min(a->ry, b->ry), c->ry); }
float DelunayTriangle::getMaxY() const { return max(max(a->ry, b->ry), c->ry); }
