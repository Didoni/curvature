...
public class Gradients {
    ...
    private void initGradients() {
        ...
        for(int i = 0; i < GRADIENT_RES; ++i){
            ...
            gradientFire[i] = marionettesim.utils.Color.argb2rgba( fireColor.getRGB() );
            
            gradientHue[i] = marionettesim.utils.Color.argb2rgba( 
                    Color.HSBtoRGB(ip, 1.0f, 1.0f) 
            );
            
            for(int j = 0; j < GRADIENT_RES; ++j){
                gradientHB[i][j] = marionettesim.utils.Color.argb2rgba( 
