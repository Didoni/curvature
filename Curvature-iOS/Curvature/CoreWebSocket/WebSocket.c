...
CFStringRef WebSocketGetIpAddress(WebSocketRef self){
    CFStringRef address = nil;
#if TARGET_OS_IPHONE
    ...
    SCDynamicStoreRef store = SCDynamicStoreCreate(kCFAllocatorDefault, CFSTR("CurvatureServer"), NULL, NULL);
    if (store) {
    ...
    if (getifaddrs(&list) >= 0) {
        for (struct ifaddrs* ifap = list; ifap; ifap = ifap->ifa_next) {
            ...
            if (ifap->ifa_flags & IFF_UP && ifap->ifa_addr->sa_family == AF_INET) {
                ...
                if (getnameinfo(ifap->ifa_addr, ifap->ifa_addr->sa_len, hostBuffer, sizeof(hostBuffer), serviceBuffer, sizeof(serviceBuffer), NI_NUMERICHOST | NI_NUMERICSERV | NI_NOFQDN) >= 0) {
                    address = CFStringCreateWithCString(kCFAllocatorDefault,hostBuffer,kCFStringEncodingUTF8);
                }
                break;
            }
        }
