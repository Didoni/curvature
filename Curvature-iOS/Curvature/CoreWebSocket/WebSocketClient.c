...
CFIndex __WebSocketClientWriteFrameWithData(WebSocketClientRef client, WebSocketFrameOpCode opCode, Boolean isMasked, UInt8 *maskingKey, CFDataRef payload) {
    CFIndex result = 0;
...
CFIndex WebSocketClientWriteData(WebSocketClientRef self, CFDataRef data) {
    CFIndex result = -1;
    if_self {
        if (data) {
...
// Write UTF-8 encoded string as a websocket frame (with text frame opcode).
CFIndex WebSocketClientWriteWithFormat(WebSocketClientRef self, CFStringRef fmt, ...) {
    CFIndex result = -1;
    if_self if (fmt != NULL) {
        va_list args;
        va_start(args, fmt);
...
Boolean __WebSocketClientWriteHandShake(WebSocketClientRef client);

void __WebSocketClientReadFrame(WebSocketClientRef self, CFReadStreamRef stream) {
...
Boolean __WebSocketClientWriteHandShake(WebSocketClientRef self) {
    Boolean success = 0;
    if (self->didReadHandShake) {
        if (!self->didWriteHandShake) {
...
Boolean __WebSocketClientHandShakeUpdateProtocolBasedOnHTTPMessage(WebSocketClientRef self) {
    Boolean result = FALSE;
    if (self) {
...
Boolean __WebSocketClientReadHandShake(WebSocketClientRef self) {
    Boolean result = FALSE;
    if_self {
